...
namespace Anni2
{
	...
	void IImageBase::TransitionImageLayoutOnQueueImmediately(vk::ImageLayout oldLayout, vk::ImageLayout newLayout, std::optional<vk::ImageSubresourceRange> subresource_range, std::optional<Queue*> queue_used_to_transition = std::nullopt) const
	{
		vk::CommandPool command_pool;
		vk::Queue       command_quque;
		...
		//queue_manager.GetSuitableQueue();


		vk::Queue       cmd_queue_used_to_do_transition{};
		vk::CommandPool cmd_pool_used_to_do_transition{};
	
		if ( last_queue_used && queue_used_to_transition)
		{

			if ( queue_used_to_transition.value()->GetQueueCap().queue_family_index != last_queue_used.value()->GetQueueCap().queue_family_index )
			{
				//TODO: queue family ownership transfer
			}


		}
		else if ( last_queue_used )
		{
			cmd_queue_used_to_do_transition =	last_queue_used.value()->GetMainCommandPool();
			cmd_pool_used_to_do_transition = last_queue_used.value()->GetMainCommandPool();
		}
		else if ( queue_used_to_transition )
		{
			cmd_queue_used_to_do_transition =	queue_used_to_transition.value()->GetMainCommandPool();
			cmd_pool_used_to_do_transition = queue_used_to_transition.value()->GetMainCommandPool();
		}
		else
		{
			//choose a queue to do this shit, and don't forget to assign last_queue_used
		}

	
		const vk::CommandBuffer command_buffer = vk::CommandManager::BeginSingleTimeCommands(command_pool, device_manager.GetLogicalDevice());
	...
	void VkImageBase::TransitionImageLayout(Sync::VkImageMemoryBarrierEnhanced mem_barrier_enhanced, const DeviceManager::CommandPoolType command_type) const
	{
	...
	IImageBase::IImageBase(GraphicsComponent& gfx_, vk::Image image_, ImageCIEnhanced& image_CI_) :
		gfx(gfx_), device_manager(gfx.DeviceMan()), image(image_), vk_image_CI(image_CI_)
	{
	}

	vk::Image IImageBase::GetImage() const
	{
		return image;
	}

	vk::Format IImageBase::GetImageFormat() const
	{
		return vk_image_CI.vk_image_CI.format;
	}

	const ImageCIEnhanced& IImageBase::GetCI() const
	{
		return vk_image_CI;
	}

	//std::shared_ptr<TimelineSemWrapper>& IImageBase::GetSemUsedToTransfer()
	//{
